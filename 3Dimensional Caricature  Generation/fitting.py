...
class AlternatingLandmarkOptimizer:
    def __init__(self, optim, horlines, iters=4, img=None, rgb_embedding=None):
        """
        ...
        if type(optim.k) == int:
            ...
            self.img = torch.zeros([1, 512, 512, 3]).float().to(self.optim.device)
            self.rgb_embedding = torch.zeros([1, self.optim.k]).float().to(self.optim.device)
        else:
            self.z = torch.zeros([1, self.optim.k[0]+self.optim.k[1]]).float().to(self.optim.device)
        # self.img = img
        # self.rgb_embedding = rgb_embedding
        self.V, self.v_rgb = self.optim.decode(self.z, self.img, self.rgb_embedding)
    ...
    def run(self, lmk):
        for _ in range(self.iters):
            ...
            self.z = self.optim.run(lmk, self.lmks_bc, self.z, self.R, self.s, self.t, self.img, self.rgb_embedding)
            with torch.no_grad():
                self.V, self.v_rgb = self.optim.decode(self.z, self.img, self.rgb_embedding)
...
class OptimizerBaseline:
    ...
    def decode(self, z, img, rgb_embedding):
        result = self.model.inference(self.V_ref, z, img, rgb_embedding)
        return result

    def run(self, lmk, lmks_bc, z_init, R, s, t, img, rgb_embedding):
        ...
        for i in range(self.max_iter):
            opt.zero_grad()
            V_new, _ = self.model.inference_back(self.V_ref, z, img, rgb_embedding)
            Vp_canon = V_new
            Vp = s * Vp_canon @ R.T + t
...
class Editing:
    def __init__(self, model, 
            V_ref, F, inds_lmk, k=128,
            lr=1e-4, criteria=1e-10, device='cuda',
            max_iter=3000, w_z=1e6, img=None, rgb_embedding=None):
        self.model = model.to(device)
        self.k = k
        self.lr = lr
        self.criteria = criteria
    ...
    def decode(self, z):
        result = self.model.inference(self.V_ref, z, self.img, self.rgb_embedding)
        return result
    ...
    def run(self, inds_vertex, pos_new, z_init, R, s, t):
        ...
        V_new, _ = self.model.inference(self.V_ref, z, self.img, self.rgb_embedding)
        V_orig = V_new.clone()[:,self.inds_lmk[inds_vertex],:]

        for i in range(self.max_iter):
            opt.zero_grad()
            V_new, _ = self.model.inference_back(self.V_ref, z, self.img, self.rgb_embedding)
            Vp_canon = V_new
            #Vp = s * Vp_canon @ R.T + t
            Vp = Vp_canon
