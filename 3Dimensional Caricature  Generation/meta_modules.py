...
class HyperNetwork(nn.Module):
    def __init__(self, hyper_in_features, hyper_hidden_layers, hyper_hidden_features, hypo_module):
        ...
        '''
        super().__init__()
        if isinstance(hypo_module, modules.SingleBVPNet):
            hypo_parameters = hypo_module.meta_named_parameters()
        elif isinstance(hypo_module, NeRFNetwork):
            hypo_parameters = hypo_module.named_parameters()
        else:
            raise TypeError(f'type of hypo_module: {type(hypo_module)}')
